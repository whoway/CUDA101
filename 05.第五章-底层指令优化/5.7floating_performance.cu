...
__global__ void float_compute(float *inputs, int N, size_t niters,
                                      float *outputs)
{
...
__global__ void double_compute(double *inputs, int N, size_t niters,
                                       double *outputs)
{
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    size_t nthreads = gridDim.x * blockDim.x;
                            ...
                            double *sample, int sampleLength)
{
    ...
    double toDeviceStart = GetCPUSecond();
    cudaMemcpy(d_doubleInputs, h_doubleInputs, sizeof(double) * N,
                     cudaMemcpyHostToDevice);
    *toDeviceTime = GetCPUSecond() - toDeviceStart;

    double kernelStart = GetCPUSecond();
    double_compute<<<blocksPerGrid, threadsPerBlock>>>(d_doubleInputs,
            N, niters, d_doubleOutputs);
    cudaDeviceSynchronize();
    *kernelTime = GetCPUSecond() - kernelStart;

    double fromDeviceStart = GetCPUSecond();
    cudaMemcpy(h_doubleOutputs, d_doubleOutputs, sizeof(double) * N,
                     cudaMemcpyDeviceToHost);
    *fromDeviceTime = GetCPUSecond() - fromDeviceStart;
